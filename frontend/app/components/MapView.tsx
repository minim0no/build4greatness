...
export default function MapView() {
    ...
    const handleDownloadResponsePdf = useCallback(async () => {
        ...
        try {
            ...
            let logoW = 0;
            let logoH = 0;
            const rawLogo = await fetchUrlAsDataUrl(
                `${window.location.origin}/logoblack.png`,
            );
            if (rawLogo) {
            ...
            const statsObj = sim.stats
                ? ({ ...sim.stats } as unknown as Record<string, unknown>)
                : null;
            await generateResponsePdf({
                ...
                location: selectedPoint
                    ? { lat: selectedPoint[1], lng: selectedPoint[0] }
                    : null,
                radiusKm,
                lastRun: lastRunMeta,
                stats: statsObj,
                ...
                infrastructureCounts: infrastructureCountsFromPayload(
                    sim.infrastructure,
                ),
            });
        } finally {
    ...
    const infrastructureMarkers: InfraMarker[] = useMemo(() => {
        ...
        return [
            ...(
                (sim.infrastructure.hospitals as Array<{
                    name: string;
                    lat: number;
                    lon: number;
                }>) || []
            ...
            ...(
                (sim.infrastructure.shelters as Array<{
                    name: string;
                    lat: number;
                    lon: number;
                }>) || []
            ...
            ...(
                (sim.infrastructure.fire_stations as Array<{
                    name: string;
                    lat: number;
                    lon: number;
                }>) || []
            ...
            ...(
                (sim.infrastructure.police as Array<{
                    name: string;
                    lat: number;
                    lon: number;
                }>) || []
            ...
            ...(
                (sim.infrastructure.ambulance_stations as Array<{
                    name: string;
                    lat: number;
                    lon: number;
                }>) || []
            ...
            ...(
                (sim.infrastructure.fuel_stations as Array<{
                    name: string;
                    lat: number;
                    lon: number;
                }>) || []
            ...
            ...(
                (sim.infrastructure.power as Array<{
                    name: string;
                    lat: number;
                    lon: number;
                }>) || []
            ...
            ...(
                (sim.infrastructure.disaster_infrastructure as Array<{
                    name: string;
                    lat: number;
                    lon: number;
                }>) || []
    ...
    const infrastructureExtrusionsGeoJSON =
        useMemo((): GeoJSON.FeatureCollection | null => {
    ...
    const renderStats = () => {
        ...
        return (
            <div className="grid grid-cols-2 gap-x-4 gap-y-1">
                ...
                <span className="font-mono">
                    {stats.total_area_km2} km² ({stats.flood_coverage_pct}%)
                </span>
                <span className="text-white/50">High-risk:</span>
                <span className="font-mono">
                    {stats.high_risk_area_km2} km²
                </span>
            </div>
        );
    };
        ...
        >
            ...
            <div className="absolute inset-0 z-10">
                ...
                >
                    ...
                    {/* Blocked roads layer */}
                    {sim.blockedRoadsGeoJSON &&
                        sim.blockedRoadsGeoJSON.features.length > 0 && (
                            <Source
                                id="blocked-roads-source"
                                type="geojson"
                                data={sim.blockedRoadsGeoJSON}
                            >
                                <Layer
                    ...
                    {infrastructureExtrusionsGeoJSON && (
                        <Source
                            id="infrastructure-extrusions-source"
                            type="geojson"
                            data={infrastructureExtrusionsGeoJSON}
                        >
                            <Layer
