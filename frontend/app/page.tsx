...
export default function LandingPage() {
  ...
  useEffect(() => {
    ...
    for (let i = 0; i < NUM_METEORS; i++) {
      meteors.push({
        startX: 0.52 + Math.random() * 0.38,
        startY: 0.05 + Math.random() * 0.34,
        travelX: 0.12 + Math.random() * 0.16,
        travelY: 0.07 + Math.random() * 0.12,
        length: 72 + Math.random() * 84,
        thickness: 1 + Math.random() * 1.8,
        delay: Math.floor(Math.random() * 340),
        duration: 34 + Math.floor(Math.random() * 24),
      });
    }
    ...
    function drawMeteor(meteor: Meteor, W: number, H: number) {
      const cycle = 360 + meteor.duration;
      const tick = (frame + meteor.delay) % cycle;
      if (tick > meteor.duration) return;

      const progress = tick / meteor.duration;
      const eased = 1 - Math.pow(1 - progress, 2.2);
      const headX = meteor.startX * W - eased * meteor.travelX * W;
      const headY = meteor.startY * H + eased * meteor.travelY * H;
      const tailX = headX + meteor.length;
      const tailY = headY - meteor.length * 0.46;

      const gradient = ctx.createLinearGradient(tailX, tailY, headX, headY);
      gradient.addColorStop(0, 'rgba(255,255,255,0)');
      gradient.addColorStop(0.35, `rgba(164, 206, 255, ${0.14 + eased * 0.22})`);
      gradient.addColorStop(1, `rgba(240, 247, 255, ${0.82 - progress * 0.18})`);
      ...
      ctx.lineCap = 'round';
      ctx.shadowBlur = 18;
      ctx.shadowColor = 'rgba(145, 205, 255, 0.54)';
      ctx.beginPath();
      ctx.moveTo(tailX, tailY);
      ctx.lineTo(headX, headY);
      ctx.stroke();

      ctx.fillStyle = `rgba(248, 251, 255, ${0.65 - progress * 0.18})`;
      ctx.beginPath();
      ctx.arc(headX, headY, meteor.thickness * 0.9, 0, Math.PI * 2);
      ctx.fill();
      ctx.restore();
    }
    ...
    function drawNormalScene() {
      ...
      brightStars.forEach((star, index) => {
        const twinkle = 0.55 + 0.45 * Math.sin(frame * 0.032 + index * 0.6);
        drawCrossStar(star.x * W, star.y * H, star.size, twinkle);
      });

      meteors.forEach((meteor) => {
        drawMeteor(meteor, W, H);
      });
      ...
      arcGlow.addColorStop(0.4, 'rgba(118, 196, 255, 0.1)');
      arcGlow.addColorStop(0.68, 'rgba(140, 216, 255, 0.24)');
      arcGlow.addColorStop(0.9, 'rgba(156, 228, 255, 0.16)');
      arcGlow.addColorStop(1, 'rgba(0, 0, 0, 0)');
      ctx.save();
      ctx.beginPath();
      ctx.arc(earthX, earthY, earthRadius * 1.083, Math.PI + 0.1, -0.1);
      ...
      ctx.save();
      ctx.beginPath();
      ctx.arc(earthX, earthY, earthRadius * 1.035, Math.PI + 0.09, -0.09);
      ctx.strokeStyle = 'rgba(166, 228, 255, 0.2)';
      ctx.lineWidth = 10;
      ctx.filter = 'blur(8px)';
      ctx.stroke();
      ctx.restore();
      ...
      ctx.strokeStyle = 'rgba(150, 225, 255, 0.72)';
      ctx.lineWidth = 2.4;
      ctx.stroke();
      ctx.restore();
    ...
    function drawWormhole(progress: number) {
      ...
      warpStreaks.forEach((streak, index) => {
        ...
        ctx.lineWidth = Math.min(10, 1 + eased * 5 + (index % 3) * 0.45);
        ctx.beginPath();
        ctx.moveTo(x1, y1);
        ctx.lineTo(x2, y2);
        ctx.stroke();
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden bg-black">
      ...
      <div
        className={`absolute inset-[10px] border pointer-events-none transition-opacity duration-500 ${
          isWarping ? 'opacity-0 border-transparent' : 'opacity-100 border-[#1a1f2e]/85'
        }`}
      />
      ...
      {!isWarping && (
        <div className="absolute inset-0 flex items-end justify-center pb-24 pointer-events-none">
          <button
            className={`pointer-events-auto ${styles.enterBtn}`}
            onClick={startWarp}
            disabled={isWarping}
          >
            Enter
